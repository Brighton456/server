...
app.post('/api/pay', async (req, res) => {
  ...
  if (gateway === 'payhero') {
    ...
    try {
      ...
      // Check if it's a DNS/network error (SwiftWallet unreachable)
      if (error.message.includes('ENOTFOUND') || error.message.includes('ECONNREFUSED')) {
        logProcess('SWIFTWALLET_UNREACHABLE', { 
          error: error.message,
          fallback: true
        }, 'WARN');
        
        console.log('⚠️ SwiftWallet API unreachable - providing fallback response');
